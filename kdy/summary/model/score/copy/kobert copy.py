...
# 문장이나 문서를 토큰화하고, 벡터로 변환합니다
document_tokenized = tokenizer(document, return_tensors="pt", padding=True, truncation=True, max_length=512)
document_embedding = model(**document_tokenized).last_hidden_state.mean(dim=1)

sentence_tokenized = tokenizer(sentence, return_tensors="pt", padding=True, truncation=True, max_length=512)
sentence_embedding = model(**sentence_tokenized).last_hidden_state.mean(dim=1)

# 문서와 문장 간의 코사인 유사도를 계산합니다
similarity_score = cosine_similarity(document_embedding.detach().numpy(), sentence_embedding.detach().numpy())
